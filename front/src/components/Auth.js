...
function Auth({ login, auth, setAuth, userList, setUserList, user, setUser, fetchUsers }) {
    ...
    const insertUser = async () => {
        try {
            ...
            await axios.post('http://localhost:5000/userlist', userData);
            await fetchUsers();
            setUser(user);
            setAuth(true);
        } catch (err) {
